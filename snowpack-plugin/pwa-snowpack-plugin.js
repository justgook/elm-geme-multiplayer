    ...
    return {
        ...
        async onChange({ filePath }) {
            ...
            if (path.basename(filePath) === "default.mustache") {
                this.partials.default = await fs.readFile(
                    "src/default.mustache",
                    "utf-8"
                )
        ...
        generateImages() {
            return pwaAssetGenerator
                .generateImages(
                    favicon.source,
                    favicon.outputFolderPath,
                    favicon.options
                )
                .then((data) => {
                    this.data = data
                    // TODO find reason why
                    this.data.htmlMeta.favicon = this.data.htmlMeta.favicon.replace(
                        ".jpg",
                        ".png"
                    )
        ...
        async getData(isDev) {
            ...
            if (isDev) {
                if (this.lastUpdate < statsInput) {
                    ...
                    const { mtimeMs: statsCache } = await fs
                        .stat(cachePath)
                        .catch((err) => Promise.resolve({ mtimeMs: 0 }))
